    ...
    public class Node : INode
    {
        ...
        public void Inside(Camera camera)
        {
            ...
            for (int i = 0; i < objDataList.Count; ++i)
            {
               // ObjManager.Instance.LoadAsync(objDataList[i]);
                UniTaskAsyncMessageBus.Inst.PublishAsync(new LoadObjMessage(){ObjData = objDataList[i]});
            }
        ...
        public void Outside(Camera camera)
        {
            ...
            for (int i = 0; i < objDataList.Count; i++)
            {
              //  ObjManager.Instance.Unload(objDataList[i].uid);
                UniTaskAsyncMessageBus.Inst.PublishAsync(new UnLoadObjMessage(){ObjData = objDataList[i]});
            }
        }
        ...
        private void CreateChild()
        {
            ...
            for (int i = -1; i <= 1; i += 2)
            {
                for (int j = -1; j <= 1; j += 2)
                {
                    Vector3 centerOffset = new Vector3(bound.size.x / 4 * i,  bound.size.y/ 4 * j,0);
